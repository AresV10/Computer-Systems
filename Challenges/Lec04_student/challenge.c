...
void build_tree()
{
  apple.name = strcpy(&apple_str[10], "apple"); 
  banana.name = strcpy(&banana_str[10], "banana");
  mango.name = strcpy(&mango_str[10], "mango");
  nectarine.name = strcpy(&nectarine_str[10], "nectarine");
  orange.name =strcpy(&orange_str[10], "orange");
  peach.name = strcpy(&peach_str[10], "peach");
  pear.name = strcpy(&pear_str[10], "pear");

  banana.left = get_apple_ptr();
  banana.right = get_pear_ptr();
  pear.left = get_nectarine_ptr();
  nectarine.left = get_mango_ptr();
  nectarine.right = get_orange_ptr();
  orange.right = get_peach_ptr();
}

/*void iter(struct BSTNode node){
  if(node.name == NULL){
      return;
    }
    strcat(print_buf, iter(*node.left));
    strcat(print_buf, node.name);
    strcat(print_buf, iter(*node.right));
...
void prep_print(struct BSTNode node)
{
    if(&node == NULL)return;
    if(node.left !=NULL)
      prep_print(*node.left);
    strcat(print_buf, node.name);
    strcat(print_buf,"\n");

    if(node.right!=NULL)
      prep_print(*node.right);
